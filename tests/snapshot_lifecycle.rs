...
fn missing_handle_does_not_create_files() {
    ...
    let directory = temporary.path().join(uuid::Uuid::new_v4().to_string());
    let handle = SnapshotHandle::from_path(&directory).unwrap();
...
mod linux {
    ...
    fn snapshot() -> (tempfile::TempDir, SnapshotHandle) {
        let temporary = tempfile::tempdir().unwrap();
        let handle = SnapshotStore::new(temporary.path())
            .unwrap()
            .create_snapshot()
            .unwrap();
        (temporary, handle)
    ...
    fn persists_metadata_and_reopens_without_changing_artifact() {
        let (_temporary, handle) = snapshot();
        let database = handle.as_path().join("db.sqlite");
        ...
        let reopened = SnapshotHandle::from_path(handle.as_path()).unwrap();
    ...
    fn independent_builds_preserve_old_snapshot() {
        let (temporary, first) = snapshot();
        let before = fs::read(first.as_path().join("db.sqlite")).unwrap();
        let second = SnapshotStore::new(temporary.path())
            .unwrap()
            .create_snapshot()
            .unwrap();
    ...
    fn rejects_unsupported_or_invalid_metadata() {
        for (sql, expected) in [
            ...
            let error = handle.info().unwrap_err();
            match expected {
...
fn cli_failure_has_no_success_payload() {
    let temporary = tempfile::tempdir().unwrap();
    let missing = temporary.path().join(uuid::Uuid::new_v4().to_string());
    let output = Command::new(env!("CARGO_BIN_EXE_sift"))
        .arg("info")
        .arg(&missing)
        .output()
